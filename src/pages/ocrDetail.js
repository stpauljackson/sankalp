...
useEffect(()=>{x();},[])
    return (<ImageBackground  source={
       ...
       {val.length!=0 ?  <Text style={{color:"black"}}><Text style={{fontSize:20,fontWeight:800}}>You can't eat this food product , because it contains{'\n\n\n'}</Text>
       <ScrollView  style={{ height: 200, marginVertical:10,marginHorizontal:10, backgroundColor: "black",borderRadius:10 }}>
       {
        removeDuplicates(val).map(x=>(<Text key={x} style={{fontSize:18,fontWeight:800,margin:10}}>{`\u2022 ${x}\n\n\n\n`}</Text>) )
        ...
        {
        removeDuplicates(val1).map(x=>(<Text key={x} style={{fontSize:18,fontWeight:800,margin:10}}>{`\u2022 ${x}\n\n\n\n`}</Text>) )
       }
        </ScrollView>

       </Text> : <Text  style={{fontSize:20,fontWeight:800, color:'black'}}>You can safely eat this product</Text>}
